...
Открывает именованный буфер — **якорь**, в позиции которого появятся все CSS/JS/fonts теги. Немедленно выводит всё, что уже накоплено в этом буфере (`addString` до `mount`), затем запускает `ob_start()` и вставляет внутренний плейсхолдер.

```php
Assets::mount('head');
// Здесь будет позиция вывода всех assets
```

**Shutdown-страховка:** при первом вызове `mount()` регистрируется shutdown-функция. Если `render()` не будет вызван до конца скрипта, все незакрытые буферы будут автоматически закрыты — assets попадут в правильную позицию (в позиции `mount`), а не в конец страницы.

### `render(string $buffer): void`

Закрывает ob-буфер, заменяет плейсхолдер из `mount()` на реальные CSS/JS/fonts теги + инлайн-контент (`addString` после `mount`), выводит итоговый HTML.

Assets всегда выводятся **в позиции `mount()`**, независимо от того, где в коде вызваны `addCss`/`addJs` и т.д.
...
Для режима `bufferization = false`. Заменяет HTML-комментарии-плейсхолдеры `<!-- Assets::mount("name") -->` накопленным контентом и assets-тегами всех буферов. Возвращает итоговый HTML.
