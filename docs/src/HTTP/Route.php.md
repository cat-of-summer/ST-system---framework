...
2. Ищет первый подходящий маршрут среди `self::routes()`: для каждого строит регулярное выражение из `pattern`. Плейсхолдеры вынимаются одним проходом (`preg_replace_callback` по грамматике `#(/)?\{(\.\.\.)?(\w+)(\?)?(?::([^{}]+))?\}#`) и заменяются на preg_quote-безопасные плейсхолдеры-«слоты»; затем в `strict_mode = true` весь остаток (литеральные части пути) экранируется `preg_quote()`, а слоты возвращаются на место уже готовыми именованными группами. Такой порядок позволяет одному коду обслуживать оба режима **и** сохранять произвольные inline-regexp нетронутыми. Виды плейсхолдеров:
   ...
   - `{name?}` → `(?:/(?P<name>[^/]+))?` — **опциональный** сегмент; сочетается с catch-all и inline-regex (`{...name?}`, `{name?:regex}`, `{...name?:regex}` — знак `?` в грамматике всегда идёт сразу после имени, перед `:regex`).
   Ведущий слеш перед плейсхолдером ловится отдельной группой (`(/)?`); для опциональных плейсхолдеров он уезжает **внутрь** необязательной группы вместе с именованной — иначе при отсутствующем сегменте в regex остался бы «зависший» обязательный `/` перед пустым местом. Для обязательных плейсхолдеров это ничего не меняет, слеш просто приклеивается перед именованной группой, как и раньше. Если сегмент не участвовал в матче, его имя не попадает в `$matches` вовсе (обычное поведение PCRE для непройденной опциональной группы), поэтому `array_intersect_key()` не добавит его в `query_params` — контроллер должен ожидать отсутствие ключа, а не пустую строку.
...
Четыре формы плейсхолдеров покрывают разные потребности — от одного сегмента до захвата всего хвоста пути, произвольных ограничений и необязательных сегментов:
...
// 4) {name?} — необязательный сегмент, слеш перед ним тоже становится необязательным
Route::get('/users/{id?}',              [UserController::class, 'index']);
//   /api/v1/users       -> "id" отсутствует в query_params (не "", а именно отсутствует)
//   /api/v1/users/42    -> id = "42"

// Сочетается с catch-all и inline-regex — "?" всегда сразу после имени, до ":regex":
Route::get('/files/{...path?}',         [FileController::class, 'serve']);
//   /api/v1/files                  -> "path" отсутствует
//   /api/v1/files/img/2026/pic.png -> path = "img/2026/pic.png"

Route::get('/users/{id?:[0-9]+}',       [UserController::class, 'index']);
//   /api/v1/users     -> "id" отсутствует
//   /api/v1/users/42  -> id = "42"
//   /api/v1/users/abc -> НЕ совпадает (сегмент присутствует, но не проходит regex)
```
