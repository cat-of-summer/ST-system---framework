...
(список на каждое поле формы), и с ними же работают правила валидации `file`, `mime`,
...
- Пропускает пустые слоты (`error === UPLOAD_ERR_NO_FILE`); все остальные записи —
  **включая битые** (`UPLOAD_ERR_INI_SIZE`, `UPLOAD_ERR_PARTIAL` и т.п.) — оборачиваются в
  объекты, а отсеиваются уже правилами `file`/`extension`/`mime` на этапе валидации.
...
Объекты правил `file` / `mime`(`mimes`) / `extension` / `filesize` собираются **здесь**, а не
...
Вызов `isValid()` в каждой проверке — это и есть «`file` подмешивается автоматически»
...
public function upload(UploadRequest $request): Response {
    // Валидация (required|file|extension|mime) уже прошла в UploadRequest::__schema()
