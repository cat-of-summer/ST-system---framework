...
Рекурсивно сканирует каталог `$dir` на `*.php`-файлы (через `File::find()`). Для каждого файла восстанавливает полное имя класса как `$namespace . '\\' . <относительный путь без .php, / заменены на \\>`, и если такой класс существует, является подклассом `Command` и имеет непустую сигнатуру (`$class::getSignature() !== ''`) — регистрирует его через `register()`. Файлы, не подошедшие под эти условия (класс не найден, не подкласс `Command`, либо сигнатура пуста), молча пропускаются.
