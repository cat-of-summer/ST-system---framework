...
| `salt` | `''` | Секрет: солит ключи кэша по IP и шифрует хранимые данные. **Обязателен** для `handleIp`/`banIp`/`unbanIp` — иначе `\LogicException` |
| `firewall.limits` | `[[60,60],[600,3600]]` | Список пар `[maxHits, windowSeconds]` — лимит обращений с одного IP в окне |
| `firewall.banTtl` | `3600` | Сколько секунд IP остаётся в бане после превышения лимита |
...
Все три метода требуют непустого `config('salt')` — иначе `\LogicException`.
...
Access::setConfig([
    ...
    'firewall' => [
        'limits'  => [[120, 60], [2000, 3600]], // ≤120 запросов/мин и ≤2000/час
        'banTtl'  => 1800
    ],
]);
...
3. если хоть один лимит превышен — ставит бан на `firewall.ttl` секунд, поднимает событие `'ban'`
   и вызывает `Access::throw(429)`.
...
Принудительно банит IP. `$ttl` (сек.) — длительность бана; `null` → `config('firewall.banTtl')`.
Поднимает событие `'ban'`. Невалидный IP → `\InvalidArgumentException`.

```php
Access::banIp('203.0.113.7');          // на firewall.banTtl
