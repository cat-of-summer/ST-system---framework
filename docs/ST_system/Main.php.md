...
Детерминированная сериализация любого значения в строку. Одинаковые данные **всегда дают одинаковый результат** (независимо от порядка ключей в массиве). Используется для построения кэш-ключей.

Поддерживает `string`, `int`, `float`, `bool`, `null`, `array`, `object`. Для `object` вызывает `jsonSerialize()` если реализован `\JsonSerializable`.

**Бросает:** `RuntimeException` если объект содержит циклические ссылки или передан неподдерживаемый тип.

```php
Main::serialize(['b' => 2, 'a' => 1]); // тот же результат, что и Main::serialize(['a' => 1, 'b' => 2])
Main::serialize(null);                // 'n'
Main::serialize(42);                  // 'i:42'
Main::serialize('hello');             // 's:hello'
```
