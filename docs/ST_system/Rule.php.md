...
Логика параметра valid
  ...
  - Дефолт применился ($valid=false): возвращает false + skip=true → цепочка рвётся, handleError нет → ошибки нет.
  Последующие правила не запускаются.  - Дефолт не применился (значение уже было): возвращает true → цепочка продолжается как обычно.
  - Дефолт не применился (значение уже было): возвращает true → цепочка продолжается как обычно.
  - $valid=true: всегда возвращает true → цепочка продолжается в обоих случаях.
  
  То есть, если $valid=true, то если $value == default, значение пропускается, иначе идут последующие проверки
