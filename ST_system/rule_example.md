# ST_system\Rule — Полная документация
...
Если условие `true` — поле должно быть пустым/отсутствовать.
...
Если условие `true` — поле устанавливается в "undefined" и **удаляется** из результирующего массива. Последующие правила не выполняются.

```php
$rule = Rule::object([
    'debug_info' => [Rule::excludeIf(!$isDevMode), 'string'],
]);
// При $isDevMode=false поле 'debug_info' будет удалено из $data
```
...
### 4.20 `Rule::init(bool $refresh = true): void` — инициализация/сброс реестра

Регистрирует все встроенные правила. Вызывается автоматически при первом обращении к любому публичному методу.

```php
// Явный сброс + перерегистрация встроенных
Rule::init(true);

// Используется для тестов — чтобы удалить пользовательские алиасы:
Rule::create(fn(&$v) => true)->alias('my_rule');
Rule::init(true); // 'my_rule' больше нет, все встроенные восстановлены

// init(false) — ничего не делает если уже инициализировано
Rule::init(false); // no-op если реестр уже наполнен
```
